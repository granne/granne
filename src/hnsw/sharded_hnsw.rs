          ...
          Element: 'a + ComparableTo<Element> + Sync
{
    ...
    pub fn search(
        ...
        let mut results: Vec<_> = crossbeam::scope(|scope| {
            let children: Vec<_> = (0..self.indexes.len()).map(|i| {
                scope.spawn(move || {
    ...
    pub fn sequential_search(
        ...
        let mut results: Vec<_> = self.indexes
            .iter()
...
mod tests {
    ...
    fn sharded_vs_sequential() {
        ...
        let elements: Vec<ElementType> = shards.iter().flat_map(|&(_, ref chunk)| chunk.clone().into_iter()).collect();
