    ...
    if let Ok(mut file) = File::open(config_file) {
        ...
        if let Ok(mut settings) = toml::from_str::<Settings>(&contents) {
            ...
            if settings.scalar == "word_id" {
                ...
                let word_embeddings_file = matches.value_of("word_embeddings").expect("scalar: query requires word embeddings file");
                ...
                if let Some(mut index_file) = existing_index {
                    builder = granne::HnswBuilder::read_index_with_borrowed_elements(
                        build_config, &mut index_file, &query_embeddings).expect("Could not read existing index");
                } else {
                    builder = granne::HnswBuilder::with_borrowed_elements(
                        build_config, &query_embeddings);
                }
